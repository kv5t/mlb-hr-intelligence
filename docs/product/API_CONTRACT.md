...
| `team` | players list, player detail/recurrence/HR log, leaderboard, games list | Canonical team UUID. For player analytics, represented-team filter precedes last N. Not accepted on team-path endpoints. |
...
| `league`, `division` | teams list | Exact sourced identity filters when available; unknown metadata does not become a fabricated division. |
...
| `players/` | `season,team,search,position,bats,ordering,page,page_size` | Paginated rows `{player:PlayerSummary,metrics?:{player.*:MetricValue},scope?:WindowScope}`; season enables attached season metrics. |
...
| `games/` | `season,date_from,date_to,team,status,ordering,page,page_size` | Paginated `GameSummary`; may show schedule and non-regular games. |
| `games/{id}/` | none | `GameDetail` with participants, HR events, domain coverage and meta. |
...
| `today/` | `season,date,window` | Date-focused games and basic descriptive leaders; both season and date required. |

`teams/` and `players/` without season return identity/profile rows only and still include `meta` for consistency; no inferred current-season KPI. With season, attached metric maps follow the same coverage/null semantics, and each row has its own scope. `games/` may span game types but any `hr_count` field obeys HR coverage. Today is a screen aggregate for an explicit `date`, not a live prediction. `window` for its leader panel uses that date as official-date cutoff; only completed eligible games through that date contribute, and incomplete results remain semantic nulls. `today/` never claims that schedule `date` and official-date cutoff are the same concept for every game.

## 6. Resource schemas (compact)

Fields marked `?` may be null or omitted where not applicable; full response examples follow. `meta` is present on all endpoint envelopes; analytical envelopes also include `scope` and `coverage` in `meta` or as named top-level sections shown below.
...
```json
{
  ...
  "meta":{"dataset_revision":"42","data_as_of":"2024-05-01T12:00:00Z","scope_kind":"TEAM_MATRIX"}
}
```

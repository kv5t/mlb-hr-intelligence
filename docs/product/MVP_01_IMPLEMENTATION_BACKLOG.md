...
Build with deterministic **synthetic canonical fixtures → domain/database → analytics → API → frontend vertical slices → exports → ingestion integration**. B01–B25 require no live provider HTTP; B22–B25 implement ingestion with deterministic fixture payloads. Only B26 may perform real-provider validation after externally recorded G12 `PROVIDER_ACCESS_APPROVED` approval for that operation. Live fetch, backfill, or operational sync remain deny-by-default until that approval; G06 PA/HR completeness is a separate hard analytics launch gate. G03/G05 can yield honest UNKNOWN matrix cells without inventing DNP or NOT_WITH_TEAM. No CI test makes a live provider request. Public 0.1 API is GET-only; internal ingestion commands are not public routes. Do not introduce Matchup, Explore, Statcast, weather, prediction, Redis/Celery, or an asynchronous export system.

The backend/domain critical chain is **B01 → B03–B06 → B07–B10 → B11 → B13**. B01 and B02 are independent foundation branches: B02 is not on that backend/domain chain, but it must complete before B12 and therefore before B13. Later screen slices B14–B20, exports B21, and fixture-driven ingestion B22–B25 follow their listed dependencies. G12 external approval enables B26; B26 establishes G06 against approved real samples. B27 reviews the integrated 0.1 release. Unapproved access does not stall the synthetic application build, but it prevents live-provider automation and production claims.

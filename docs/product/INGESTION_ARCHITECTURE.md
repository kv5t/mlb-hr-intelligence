...
`data_as_of` is the committed `DatasetRevision` and timestamp, never the provider game time. `DatasetRevision` is an MVP **consistency/publication token**, not a promise that arbitrary historical versions can be queried. Web/CSV/PDF must read one consistent current revision for an entire response/export, using a database read snapshot or retry if the current revision changes mid-read. Historical `?dataset_revision=X` reads are unsupported in 0.1 unless a later versioned-storage design explicitly enables them. A no-op fetch does not increment revision. The revision history keeps changed target and source references so results can be explained without full database snapshots.

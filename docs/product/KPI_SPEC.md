...
**Reason precedence:** unresolved selection/coverage (`INCOMPLETE`, `UNKNOWN`, `ORDER_UNVERIFIED`) first; then mathematical reasons (`NO_GAMES`, `ZERO_DENOMINATOR`, `NO_HOME_RUNS_IN_SCOPE`, `INSUFFICIENT_HISTORY`). A complete empty scope permits HR, PA, HR Games and multi-HR games to be numeric 0; game rates, gaps, droughts and streaks have `NO_GAMES` or another specified reason. If all games are complete but `H=0`, game/PA droughts are the scope length with `NO_HR_IN_SCOPE` annotation, not a fabricated pre-window HR.
...
- **Definition / formula / numerator / denominator:** Length of trailing run of `b_i=0` in W through cutoff; numerator trailing non-HR game count; denominator none. If `B=0,m>0`, value `m` with `NO_HR_IN_SCOPE` annotation; never search before W.
- **Eligible set / required coverage:** Ordered `G_s(W)`; known membership and HR coverage from the latest game backward through the trailing non-HR run and its first preceding HR, or to the start of W. A complete latest HR alone proves value 0. Verify order where material.
- **Window / cutoff / filters:** Shared W, inclusive cutoff and pre-window filters; latest **selected** game determines current state.
...
- **Definition / formula / numerator / denominator:** In ordered canonical PAs across W, count trailing PAs without an HR event; numerator trailing non-HR PA count; denominator none. If no HR but `P>0`, value `P` with `NO_HR_IN_SCOPE` annotation.
- **Eligible set / required coverage:** Player `G(W)` and their PAs; complete player PA and game HR coverage plus known `game_pa_ordinal` for all relevant PAs.
- **Window / cutoff / filters:** Shared player W/cutoff/pre-window filters; PA run is confined to selected game window, not a separately chosen last-N-PA window.
...
- **Definition / formula / numerator / denominator:** Length of trailing run of `b_i=1` in W; numerator trailing HR-game count; denominator none.
- **Eligible set / required coverage:** Ordered `G_s(W)`; known membership and HR coverage from latest game backward through trailing HR run and its first preceding non-HR, or to start of W. A complete latest non-HR alone proves value 0. Verify order where material.
- **Window / cutoff / filters:** Shared W, inclusive cutoff and pre-window filters; latest selected eligible game controls current state.

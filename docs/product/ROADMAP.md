...
**Status:** Updated through Phase 0.0-C. Current phase: 0.0-D (provider research documented; launch gates remain blocked in PROVIDER_GAPS.md). A phase is complete only when its exit criteria are met. Later features are not implicitly authorized by completion of an earlier phase.
...
- **In scope:** 0.0-A product scope/screen map; 0.0-B canonical data model; 0.0-C KPI dictionary/formulas; 0.0-D provider verification/mapping (current phase); 0.0-E ingestion architecture; 0.0-F DRF API contracts; 0.0-G frontend architecture; 0.0-H final 0.1 backlog.

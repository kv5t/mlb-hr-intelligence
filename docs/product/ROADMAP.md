...
**Status:** Updated through Phase 0.0-H. 0.0 specification complete; 0.0-G frontend architecture complete. Next phase: 0.1 — HR Recurrence implementation. Provider access and coverage launch gates remain open. A phase is complete only when its exit criteria are met. Later features are not implicitly authorized by completion of an earlier phase.
...
- **In scope:** 0.0-A product scope/screen map; 0.0-B canonical data model; 0.0-C KPI dictionary/formulas; 0.0-D provider verification/mapping; 0.0-E ingestion architecture; 0.0-F DRF API contracts; 0.0-G frontend architecture (complete); 0.0-H final 0.1 backlog (complete).
